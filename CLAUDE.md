...
| 一対（いっつい） | ダイレクトメッセージ | DM | 1対1の会話空間（**旧称: 縁側**） |
...
アプリ設定で用語セットを切り替え可能にする仕様（`preferences.terminology`）がある。UI文言はハードコードせず切り替え可能な構造にする。`docs/マップ.md`では「世界観重視／利便性重視」×「日本語／English」の2軸で整理されており、旧来の「LINE風／Discord風」という競合アプリ準拠の切り替え軸から変わりつつある可能性がある。この設定モデル自体の仕様（`terminology`列挙値をどう再設計するか）は未確定のため、実装前にユーザーに確認すること。

> ⚠️ **`docs/マップ.md`と本ファイル本文の間で未解決の食い違いが3点ある**（用語の言い換えではなく機能・ポリシーレベルの話なので、無断で決め打ちせずユーザーに確認してから進めること）:
> 1. **談論（プロジェクトボード型フォーラム）**: 下記「実装しない機能」表の「Discordフォーラム機能」と概念が重なる。実装するなら方針転換が必要。
> 2. **バックアップコード**: `docs/マップ.md`の設定画面案ではセキュリティ項目に「バックアップコード」があるが、下記「重要な仕様・制約」では秘密の質問方式を採用しバックアップコードは不採用と明記している。
> 3. **メールアドレス**: `docs/マップ.md`の設定画面案ではアカウント項目に「メールアドレス」があるが、本文冒頭・「実装しない機能」表では「メールアドレス・電話番号登録」はプライバシー方針上収集しないとしている（表示専用の任意欄なのか登録必須項目なのかで意味が変わる）。
...
### UIスタイル切り替え（2026-07-21実装）

設定タブから、複数のUIスタイル（見た目のテーマ）を切り替えられる機能を実装済み（`lib/models/ui_style.dart`, `lib/theme/app_theme.dart`, `lib/providers/ui_style_provider.dart`）。端末のSharedPreferencesに永続化（アカウント間・端末間の同期はまだ未対応）。

- **DaiDai（標準）**: 上記の橙色基調スタイル
- **シンプル**: mobbin.com「one year」（iOSアプリ）を参考にしたミニマルスタイル。ラベンダー系背景＋紫がかった藍色アクセント、等幅フォント、装飾を減らした余白多めのレイアウトを採用。参考アプリ独自の手書きドット/イラストアイコン素材は模倣していない（配色・フォント・レイアウトの方向性のみ参考にした）

新しいUIスタイルを追加する場合は`UiStyle` enumに追加し、`AppTheme.themeFor()`に対応する`ThemeData`を実装する。
