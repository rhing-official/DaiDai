...
イメージキャラクター・アプリ本体・ロゴの著作権は、Rhing及び代表（畑中新）に帰属する。詳細は`docs/DaiDai-企画書.md`の「1.1 基本情報」参照。

  

## 実装前の確認・質問方針（2026-09-24追加）

  

2026-09-14に「タブレットを横向きにするとアイコン＋寄合一覧レイアウト設定（`TalksListLayoutStyle.iconSplit`）が無視される」不具合を修正した際、判定条件（`talks_tab.dart`の`_isSplit`、画面サイズのみで判定しプラットフォーム種別を区別できない）がPCとタブレット横向きを区別できないことに気づかないまま対応した結果、意図せずPCでも「この設定を適用しない」という当初仕様（2026-09-11導入時から）を崩してしまった。この変更はユーザーへの確認なく行われ、日記.md本文にも独立した説明が記載されず、コード内コメントにのみ経緯が残る形になったため、10日後にユーザーから指摘を受けるまで気づかれなかった（2026-09-24に`classifyDevice()`を併用する形で修正、詳細は`日記.md`参照）。この教訓を踏まえ、以後は以下を徹底する。

  

- **既存の仕様・挙動を変更する際は、着手前に変更が影響する範囲を洗い出す**。修正対象の判定条件・フラグが、意図した対象以外（例: タブレット横向きの修正のつもりが広い画面全般＝PCも含む対象に及ぶ等）にも同時に影響しないか確認する。画面幅・アスペクト比のみに基づく判定はプラットフォーム種別を区別できない点に注意し、PC/タブレット区別が必要な場合は`lib/utils/platform_info.dart`の`classifyDevice()`のような既存の判定手段を使う

- **既存の仕様に疑問・不明点がある場合は、自己判断で仕様を変えずに必ずユーザーに確認する**。「不具合に見えるが、実は意図した仕様かもしれない」というケースでは、修正前に確認を取ってから進める

- **実装した内容は日記.mdの「実装内容」に、変更の意図・影響範囲が伝わる形で明記する**。コミット内のコードコメントのみに経緯を残し、日記本文への記載を省略しない

2026-09-25、「寄合機能オフ時にハンバーガーメニューを飛ばす」実装を依頼された際、指示通りに実装すると広場の「退会」導線が失われることに気づいたため、以下も徹底する（この時はAskUserQuestionで確認してから対応した）。

- **指示された範囲を超える変更は行わない**。既存の関連コードに触れている最中に「ついでに直したくなる」「様子見でこうしておく」といった変更も対象に含む
- **どうしても指示範囲外の変更が避けられない場合は、実装前にその旨を明示してユーザーに確認を取る**。「実装後についでにこれも直しておきましたと報告する」だけでは不十分。ユーザーが提案内容を誤解したまま意図せず許可してしまわないよう、①何が指示範囲外か、②なぜそれが必要か（指示通りに実装した場合に何が失われる/壊れるか）、③対応しない場合どうなるか、を分けて明確に伝える

2026-09-26、アルバムへ「メッセージから追加」する一覧画面を新規実装した際、`Navigator.push`によるフルスクリーン別ルートで開いてしまい、寄合の表示領域（広い画面ではフレンド/寄合一覧サイドバーの隣のペイン部分のみ）を超えて画面全体を覆う不具合をユーザーから指摘された。`AlbumPaneView`自体は`NotePaneView`/`CalendarPaneView`と同じ「親のStateが持つローカルなbool/オブジェクトの切り替えでペイン内容を差し替える」方式（`Navigator.push`を使わない）を採用しており、その理由（サイドバーまで覆ってしまう不具合を過去に修正した経緯）がドキュメントコメントに明記されていたにもかかわらず、新規追加した一覧画面だけこの方式を踏襲していなかった。

- **「メッセージ画面」（寄合の表示領域、`lib/features/chat/chat_panes.dart`の`DmChatPane`/`GroupChatPane`が占めるペイン）の中から開く一覧・選択・設定画面は、必ずそのペインの表示領域内に収まる形で表示する**。`AlbumPaneView`/`NotePaneView`/`CalendarPaneView`が採用している「親Stateのローカルなbool/オブジェクトフィールドを切り替え、`build()`で差し替える」パターンに倣うこと。`Navigator.push`によるフルスクリーン別ルートは、広い画面の分割表示（フレンド一覧・寄合一覧サイドバー）を全て覆ってしまうため使わない
- **例外**: 写真・動画そのものを全画面で閲覧するビューア（`AlbumMediaViewerScreen`、チャット添付ファイルの`media_viewer_screen.dart`等）は対象外。メディアを拡大表示する体験として全画面が既存の意図した挙動であり、この方針の対象は一覧・選択・設定など「ペインの中身の差し替え」に相当する画面に限る
- 新しく「メッセージ画面内に何かを表示する」画面を追加する際は、着手前にこの区別（ペイン内容の差し替えか、独立した全画面ビューアか）を判断し、前者であれば既存のローカル状態切り替えパターンを踏襲する
...
| ぺったん | スタンプ | スタンプ | スタンプ機能（**旧称: ペタピタ**、2026-09-12変更。ユーザーから「言いづらい」との指摘を受け、世界観用語の路線は維持しつつ発音しやすい表記に変更した） |
...
- **Sticker（ぺったん、`StickerPack.stickers[]`）**: `stickerId`, `name`, `imageUrl`, `roles[]`（メッセージ内容に応じたぺったん提案で使う役割id、2026-09-05追加。詳細は下記「メッセージ内容に応じたぺったん提案機能」参照）
...
### メッセージ内容に応じたぺったん提案機能（2026-09-05追加）

  

入力中のメッセージ本文にキーワードが含まれると、入力欄のすぐ上に該当するぺったんの候補が横並びで表示され、タップで即送信できる（`lib/features/chat/sticker_suggestion_strip.dart`、`chat_screen.dart`の`_onComposerTextChanged`から400msデバウンスで`lib/utils/sticker_suggestion.dart`の`suggestStickers`を呼ぶ）。LINEの「おすすめスタンプ」機能の設計（クリエイターがスタンプ1枚ごとにタグを設定し、そのタグ経由で提案する方式）を踏襲したもので、DaiDaiでは「役割（`StickerRole`、感情・場面のカテゴリ）→キーワード→ぺったん（`Sticker.roles`）」という中間層で実装している（採否の根拠は競合調査.md 2026-09-05付エントリ参照）。形態素解析等は導入せず、ひらがな/カタカナ正規化＋大文字小文字統一をした上での単純な部分一致に留めている（表記ゆれ対応の精度をLINE並みに最初から作り込むのはコスト高なため、まず簡易な一致から始める方針）。提案ストリップのタップは、通常のぺったんピッカー（`StickerPickerContent`）が持つ`stickerSendMode`のLINE型2タップ・プレビュー挙動とは独立に、常に即送信にしている。

  

DaiDaiには現状ぺったんパックの「作成」UI自体が無く（下記フェーズ表の通りフェーズ4「ぺったん作成アプリ（別アプリ）」として別アプリに切り出す計画）、`StickerRole`の初期セット（10種、`functions/src/index.ts`の`seedStickerRolesOnce`に叩き台を実装）はCloud Functionsの一度きりの処理でFirestoreへ投入する想定。既存の各ぺったんへの`roles`の割り振りは画像内容の確認が要る人力作業のため、この一度きり処理には含めておらずFirestoreコンソールから個別に設定する（Cloud Functions側の`createStickerPack`/`addStickersToStickerPack`は`roles`を受け付けるようスキーマだけ対応済み、2026-09-05）。
...
| フェーズ4（将来） | 未定 | AI搭載メッセージ整理、ぺったん作成アプリ（別アプリ）、貼プラン |

  

### 極みプランの無料先行公開（2026-09-14方針決定、未着手）

2026-09-14時点で`subscriptionPlan`/極み関連のコードはリポジトリ全体で未実装（0件）。以下は実装前の方針決定であり、着手時はこの内容を踏まえること。

- **背景**: リリース後半年の認知獲得ロードマップ（上記「リリース後半年の認知獲得ロードマップ」参照）で、Web版ソフトローンチ期間中にできるだけ多くの機能をテストしてもらい不具合を出し切りたいという狙いから、極みプランの機能自体を前倒しでフェーズ1に含めることにした。
- **課金ゲートは切り離す**: 極みプラン（既読非表示・1080p通話・極み限定カラー/フォント等）は機能としては実装するが、Stripeでの実際の課金は行わず、当面は`subscriptionPlan`を問わず全住人に無料開放する。これは有料ぺったんパック販売を`functions/src/index.ts`の`PAID_PACKS_ENABLED`フラグで一時凍結している既存パターン（特商法表記・返金ポリシー・Stripe本番アカウント等の商取引の手続きが未整備なため）と同じ設計思想で、極みプランについても同様のフラグ（例: `KIWAMI_BILLING_ENABLED`相当）で課金ゲートのみを後から有効化できるようにする想定。
- **再ゲート時の注意**: ぺったんの有料パック凍結は「そもそも有料パックが買えない」だけなので後から有料化しても違和感が薄いが、極みプランは既読非表示・1080p通話など**体験そのものが変わる機能**を無料開放することになるため、後から課金ゲートを掛けると「機能を取り上げられた」という反発を招きやすい。実装時は、UI上に「ローンチ記念で期間限定無料開放中」等、いずれ課金化されうることを明示する文言を入れておくこと。
...
### リリース更新頻度の方針（2026-09-15決定）

ブラウザ版は新機能実装・不具合修正のたびに随時更新する。アプリ版（Android/Windows/Linux/iOS/macOS）は、重大な脆弱性対応など緊急を要する場合を除き、週1回まとめて更新する。詳細・背景は`docs/DaiDai-企画書.md`の「8. プラットフォーム戦略」参照。

## リリース後半年の認知獲得ロードマップ（2026-09-14策定）

Google Play Consoleの本人確認は、住所の表記ゆれでパスポート提出が一度差し戻された。運転免許証（2026-09-14時点で仮免許、本免許まで約3ヶ月）の交付を待つ方針に切り替え済み。この待機期間を、Web版の品質固めと住人（ユーザー）獲得に充てる計画（詳細な費用感・候補チャンネルの出典は`競合調査.md`の「2026-09-14 ブラウザ版先行運用期間のアーリーアダプター獲得施策」参照）。

### フェーズ1（1ヶ月目）: 立ち上げ月 — Web版ソフトローンチ

最初の住人に、率直な不具合報告をもらうことを最優先にする。

**1週目**
- [ ] Web版を本番URLで公開する
- [ ] 不具合報告の受け口を1つ決めて案内する（Discordサーバー／Googleフォームのどちらか）
- [ ] Zennかnoteに1本目の記事を書く：「メールアドレスも電話番号も集めないメッセージアプリを、なぜ作っているか」
- [ ] X（Twitter）アカウントで最初の告知ポストをする

**2週目**
- [ ] 「オンライン・エンジニアカフェ」「kojin-dev」等、個人開発者向けのコミュニティで告知する
- [ ] 縁結び（招待リンク・QRコード）で身近な知人に直接声をかけ、最初の住人を増やす
- [ ] 届いた不具合報告に対応し、`flutter analyze`・`flutter test`を通した上で直す

**3週目**
- [ ] 2本目の記事を書く：「公開1週間で分かった不具合・反応まとめ」
- [ ] X上で進捗を継続的に発信する（ビルドインパブリック）

**4週目**
- [ ] 1ヶ月の振り返り：獲得住人数・不具合報告の件数を数える
- [ ] Product Hunt掲載の準備（画像・紹介文）を検討する

### フェーズ2（2〜3ヶ月目）: 底固めの2ヶ月 — 免許証交付待ち

どうせ免許証を待つしかない期間。だからこそブラウザ版の完成度を上げることに使う。

- [ ] 隔週で開発ログ記事を書き続ける（Zenn／note）
- [ ] コミュニティでの告知を継続し、新しい住人からの不具合報告を集め続ける
- [ ] 運転免許証が交付され次第、Google Play Consoleの本人確認をやり直す
- [ ] Android／Windows／Linux／iOS／macOS、それぞれの最終動作確認を進める
- [ ] Web版で見つかった不具合の累積リストを潰しきる

### フェーズ3（3〜4ヶ月目）: 全プラットフォーム同時リリース

本人確認が通り次第、ここに入る。

- [ ] Google Play Consoleの審査を申請する
- [ ] 他ストア（該当があれば）の申請を進める
- [ ] 同時リリースの告知記事を書く：「全プラットフォームで使えるようになりました」
- [ ] Web版住人にも便り（公式アカウント）でリリースを知らせる
- [ ] Web版での実績（住人数・改善した不具合の数）を手土産に、ゆるコンピュータ科学ラジオ等への協賛打診を検討する

### フェーズ4（4〜6ヶ月目）: 定着と拡大

ここまでの反応を見て、有料施策に進むかを判断する。

- [ ] 無料施策の反応（記事の保存数・コミュニティでの反応・招待経由の住人数）を振り返る
- [ ] 反応が良ければ、テック系チャンネルへの協賛を実施する
- [ ] 反応が弱ければ、無料施策を継続・強化する
- [ ] 住人数・継続率を定点観測する仕組みを作る
- [ ] 6ヶ月地点で全体を振り返り、次の半年の方針を決める
...
- **マテリアルに影（`boxShadow`等）を一切使わない**（2026-08-29決定）。`GlassSurface`（`lib/widgets/glass/glass_surface.dart`）に影を描く仕組み自体が無く、面の境界は縁ストローク（`enableEdgeStroke`、`BorderRadius.zero`の全面パネルでは直線が不要な線に見えるため無効化）と`BackdropFilter`によるぼかし（`chrome`/`floating`バリアント）のみで表現する。同色の背景に全面パネルが溶け込んで見えないようにするには、`card`（ぼかし無し）ではなく`chrome`/`floating`を選ぶこと。ハンバーガーメニューのアイコンのみ影を残す例外（`GlassIconBadge`の`shadow`パラメータ、2026-08-30追加）を設けていたが、2026-09-06にユーザー指示で撤回し`shadow`パラメータごと削除した。現在は例外なく「影を一切使わない」方針を貫いている。今後同様の影の追加依頼があっても、無断で例外を復活させず都度確認すること。
