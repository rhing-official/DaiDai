...
/// （`app_theme.dart`）のフルスクリーン画面遷移専用だったものを、ホーム画面の
/// タブ切り替え（`AnimatedIndexedStack`）・設定/身だしなみのドリルダウン
/// （`settings_tab.dart`/`profile_tab.dart`の`AnimatedSwitcher`）でも同じ
/// モーション言語になるよう共通化した（2026-08-10追加）。
...
/// [child]を作る。フルスクリーン遷移・タブ切り替え・ドリルダウン全てで
/// この関数を共有する。
